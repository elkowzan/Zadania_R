...
corpus <- tm_map(corpus, removeWords, c("flight", "lot", "thats"))
...
associations <- findAssocs(tdm, target_word1, corlimit = cor_limit)
assoc_vector <- associations[[target_word1]]
...
ggplot(assoc_df, aes(x = score, y = reorder(word, score))) +
  geom_segment(aes(x = 0, xend = score, y = word, yend = word), color = "#a6bddd", size = 1.2) +
  geom_point(color = "#1aaaea", size = 4) +
  geom_text(aes(label = round(score, 2)), hjust = -0.3, size = 3.5, color = "#1A1A3A") +
...
ggplot(assoc_df, aes(x = score, y = reorder(word, score), color = score)) +
  ...
  geom_point(size = 4) +
  geom_text(aes(label = round(score, 2)), hjust = -0.3, size = 3.5, color = "#1A1A3A") +
  scale_color_gradient(low = "#a6bddd", high = "#1A1A3A") +
  scale_x_continuous(
...
## Słowo "july" bo mam wtedy urodziny ----
...
# Oblicz asocjacje dla tego słowa
associations <- findAssocs(tdm, target_word2, corlimit = cor_limit)
assoc_vector <- associations[[target_word2]]
assoc_sorted <- sort(assoc_vector, decreasing = TRUE)
...
# stosowany w raportach biznesowych i dashboardach:
ggplot(assoc_df, aes(x = score, y = reorder(word, score))) +
  geom_segment(aes(x = 0, xend = score, y = word, yend = word), color = "#FF6A6A", size = 1.2) +
  geom_point(color = "#8B3A3A", size = 4) +
  geom_text(aes(label = round(score, 2)), hjust = -0.3, size = 3.5, color = "#4A1A1A") +
  scale_x_continuous(limits = c(0, max(assoc_df$score) + 0.1), expand = expansion(mult = c(0, 0.2))) +
  theme_minimal(base_size = 12) +
  labs(
...
ggplot(assoc_df, aes(x = score, y = reorder(word, score), color = score)) +
  ...
  geom_point(size = 4) +
  geom_text(aes(label = round(score, 2)), hjust = -0.3, size = 3.5, color = "#4A1A1A") +
  scale_color_gradient(low = "#FF6A6A", high = "#8B3A3A") +
  scale_x_continuous(
    limits = c(0, max(assoc_df$score) + 0.1),
    expand = expansion(mult = c(0, 0.2))
  ) +
